...
Note that if you don't mind a very small speed hit in production, you can
set the :cooldown option to nil when instantiating the Unreloader, which
will cause it not to reload files automatically.  This allows you to
remove conditionals from your code:

    dev = ENV['RACK_ENV'] == 'development'
    require 'rack/unreloader'
    Unreloader = Rack::Unreloader.new(:cooldown=>(1 if dev)){App}
    Unreloader.require './models.rb'
    Unreloader.require './app.rb'
    run Unreloader

And to load dependencies outside of your +config.ru+:
